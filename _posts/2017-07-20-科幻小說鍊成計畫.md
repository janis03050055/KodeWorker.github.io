...
- 文件數量：1324
    ...
    2. 981
- 爬蟲時間：09:56:42
    1. 2017-07-20 16:05:27 ~ 2017-07-20 18:11:50 (2:06:23)
    2. 2017-07-21 08:14:07 ~ 2017-07-21 16:04:26 (7:50:19)
- 檔案大小：252 MB
    1. 66.8 MB
    2. 185 MB

網頁上顯示全部的英文科幻小說有1340筆，但是下載下來的`.epub`檔案僅有1324筆差距16筆，差距的資料應當寫一隻程式來進行比對。
雖然差距的資料不多，但是本人身為一個好奇寶寶，還是得找出其中差異的原因。
...
文檔的正規畫好比於資料的前置處理，依照之前的經驗，這部分應該會花費掉最多的功夫，預想大致的流程如下。
...
首先先fork到自己的Github，接著在專案中引入`ebooklib`。
...
我們視解析後的檔案大小若小於`1 KB`，則認定為檔案異常，並且以`logging`紀錄異常，以及最小正常的檔案。

(以下待更新...)

![解析資料大小分析](/assets/images/計畫/parsed_file_stats.jpg){: .align-center}
